...
const mergedList = computed<Article[]>(() => {
  const pinned = pinnedStore.tech.map((item) => ({ ...item, isTop: true }));
...
<template>
  <div class="core-article-box">
    ...
    <div class="title-list">
      <div v-for="(item, index) in mergedList" :key="index" class="article-item" @click="goToArticle(item.id)">

        <!-- 左侧：标题 (自动截断) -->
        <span class="title-text">{{ item.isTop ? '[置顶]' : '' }}{{ item.articleName }}</span>
